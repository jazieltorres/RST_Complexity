...
void MultiDimArray<F,m>::RST() {
    ...
    NTL::mat_ZZ_p X_temp, X;
    long num_cols = exponentsColumn.size();
    X.SetDims(0, num_cols);
    vector<F> r;
...
// TEST BLOCK
        r = rowPiAlpha<F,m>(alpha, A, exponentsColumn, e_period);
        X_temp.SetDims(X.NumRows(), X.NumCols());
        for (long i=0; i<X.NumRows(); i++) {
            for (long j=0; j<X.NumCols(); j++) {
                X_temp[i][j] = X[i][j];
            }
        }
        X.SetDims(X_temp.NumRows()+1, X_temp.NumCols()+1);
        for (long i=0; i<X_temp.NumRows(); i++) {
            for (long j=0; j<X_temp.NumCols(); j++) {
                X[i][j] = X_temp[i][j];
            }
        }
        ...
        for (long j=num_cols; j<X.NumCols()-1; j++){
            X[X.NumRows()-1][j] = (F)0;
        }
        X[X.NumRows()-1][X.NumCols()-1] = (F)1;
// ^ TEST BLOCK ^
...
//        if(isZero) {
            ...
            // TEST BLOCK
            X.SetDims(X.NumRows()-1, X.NumCols());
            // ^ TEST BLOCK ^
...
//            cout << endl;
        ...
        for(long i=0; i<matrix.size(); i++){
            for (long j=0; j<matrix[0].size(); j++){
                if(X[i][j] != matrix[i][j]) equal = false;
            }
        }
        cout << equal << " : ";
        equal = true;
        for(long i=0; i<idMatrix.size(); i++){
            for (long j=0; j<idMatrix[0].size(); j++){
                if(X[i][j+num_cols] != idMatrix[i][j]) equal = false;
            }
        }
        cout << equal << endl;
